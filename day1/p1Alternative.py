...
def busca2020():
    for i in range(0,len(Lista_de_numeros)):
        print(str(2020 - Lista_de_numeros[i]))
        num = busquedaDicotomica(2020 - Lista_de_numeros[i], 0, len(Lista_de_numeros)-1)
        print(str(Lista_de_numeros[i]) +" + "+ str(num) +" = "+ str(Lista_de_numeros[i] + num))
        if (Lista_de_numeros[i] + num == 2020):
            return Lista_de_numeros[i] * num
